...
def load_splirent_model(sequence_padding, drop=0.2) :
    ...
    layer_dense_hek = Dense(256, activation='relu')
    layer_drop_hek = Dropout(drop)
    layer_dense_mcf7 = Dense(256, activation='relu')
    layer_drop_mcf7 = Dropout(drop)

    concat_out = Concatenate(axis=-1)([shared_out_1, shared_out_2])

    dense_out_hek = layer_dense_hek(concat_out)
    dropped_out_hek = layer_drop_hek(dense_out_hek)
    dense_out_mcf7 = layer_dense_mcf7(concat_out)
    dropped_out_mcf7 = layer_drop_mcf7(dense_out_mcf7)
    ...
    pred_usage_hek = layer_usage_hek(dropped_out_hek)
    pred_usage_mcf7 = layer_usage_mcf7(dropped_out_mcf7)
    ...
    return [ ('splirent_only_random_regions_drop_' + str(drop).replace(".", "") + '_sep_hek_mcf7', splicing_model), ('loss', loss_model) ]
