...
def load_splirent_model(sequence_padding, drop=0.2) :
    ...
    concat_out = Concatenate(axis=-1)([dropped_out, lib_input])
    ...
    pred_usage_hek = layer_usage_hek(concat_out)
    pred_usage_hela = layer_usage_hela(concat_out)
    pred_usage_mcf7 = layer_usage_mcf7(concat_out)
    pred_usage_cho = layer_usage_cho(concat_out)
    ...
    return [ ('splirent_aligned_batchnorm_drop_' + str(drop).replace(".", ""), splicing_model), ('loss', loss_model) ]
